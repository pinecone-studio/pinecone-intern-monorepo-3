...
describe('verifyOtp function', () => {
  ...
  it('should throw error if OTP not found', async () => {
    ...
    await expect(verifyOtp(null, { verifyOtp: { otp: mockOtp } })).rejects.toThrow('OTP код олдсонгүй');
  });
  ...
  it('should throw error if OTP expired', async () => {
    ...
    await expect(verifyOtp(null, { verifyOtp: { otp: mockOtp } })).rejects.toThrow('OTP кодын хугацаа дууссан байна');
  });
  ...
  it('should throw error if user not found', async () => {
    ...
    await expect(verifyOtp(null, { verifyOtp: { otp: mockOtp } })).rejects.toThrow('Хэрэглэгч олдсонгүй');
  });
