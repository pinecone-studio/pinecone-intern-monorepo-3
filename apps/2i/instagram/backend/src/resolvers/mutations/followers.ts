...
import { addFollower } from './add-followers';
...
export const sendFollowRequest = async (_: unknown, { userId }: { userId: string }, context: { user?: { id: string } }): Promise<IUserResponse> => {
  ...
  if (targetUser.isPrivate) {
    const res = await addFollowRequest(targetUser, currentUser._id);
    await createNotification(targetUser._id.toString(), currentUserId, `${currentUser.username} sent you a follow request`);
    return res;
  } else {
    const res = await addFollower(targetUser, currentUser);
    await createNotification(targetUser._id.toString(), currentUserId, `${currentUser.username} sent you a follow request`);
    return res;
  }
};
...
export const respondFollowRequest = async (_: unknown, { userId, accept }: { userId: string; accept: boolean }, context: { user?: { id: string } }): Promise<IUserResponse> => {
  const currentUserId = checkAuthentication(context);
  const currentUser = await getUserById(currentUserId);
  ...
  return {
    ...
    user: convertPopulatedUser((await UserModel.findById(currentUser._id).populate('followers').populate('following').populate('followRequests').lean()) as unknown as IPopulatedUser),
    token: '',
  };
};
export const unfollowUser = async (_: unknown, { userId }: { userId: string }, context: { user?: { id: string } }): Promise<IUserResponse> => {
  const currentUserId = checkAuthentication(context);
  if (userId === currentUserId) throw new Error('You cannot unfollow yourself');
  await Promise.all([UserModel.findByIdAndUpdate(currentUserId, { $pull: { following: userId } }), UserModel.findByIdAndUpdate(userId, { $pull: { followers: currentUserId } })]);
  const populatedTargetUser = (await UserModel.findById(userId).populate('followers').populate('following').populate('followRequests').lean()) as unknown as IPopulatedUser;
  if (!populatedTargetUser) throw new Error('Target user not found');
  return {
