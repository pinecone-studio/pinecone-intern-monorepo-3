...
//   await expect(updateHotel(undefined, {
//     hotelName: "Test",
//     location: "UB"
//   } as any)).rejects.toThrow("No ID provided");
// });
...
//     it("it should update and result should be returned", async ()=>{
...
describe("updateHotel", () => {
  ...
  it("should throw an error if no ID is provided", async () => {
    await expect(
      updateHotel(undefined, { hotelName: "Test", location: "UB" } as any)
  ...
  it("should throw an error if hotel is not found", async () => {
    ...
    await expect(
      updateHotel(undefined, { id: "999", hotelName: "Test Hotel" })
    ).rejects.toThrow("Hotel not found");
  });

  it("should throw an error if no fields to update are provided", async () => {
  await expect(
    updateHotel(undefined, { id: "some-id" })
